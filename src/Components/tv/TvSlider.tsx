...
import { getTvshows, IGetContent } from '../../api';
...
const TvSlider = ({ subject }: ISliderProps) => {
    const navigate = useNavigate();
    const { data, isLoading } = useQuery<IGetContent>(['tvshows', `${subject}`], () => getTvshows(subject));
    const [index, setIndex] = useState(0);
    const [leaving, setLeaving] = useState(false);
    const onDetailClick = (tvId: number) => {
        navigate(`/tvshows/${tvId}/${subject}`);
    }
    const increaseIndex = () => {
        if (data) {
            if (leaving) return;
